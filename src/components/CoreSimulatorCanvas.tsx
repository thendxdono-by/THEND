...
const getDayNightGradients = (hour: number) => {
  const anchors = [
    { hr: 0,  c1: {r:3, g:7, b:18},    c2: {r:15, g:10, b:40} },
    { hr: 6,  c1: {r:180, g:65, b:20},  c2: {r:90, g:35, b:110} },
    { hr: 12, c1: {r:12, g:125, b:215}, c2: {r:215, g:145, b:15} },
    { hr: 18, c1: {r:200, g:45, b:30},  c2: {r:135, g:30, b:165} },
    { hr: 21, c1: {r:40, g:25, b:110},  c2: {r:10, g:12, b:35} },
    { hr: 24, c1: {r:3, g:7, b:18},    c2: {r:15, g:10, b:40} },
  ];
  ...
  return {
    color1: `rgba(${finalC1.r}, ${finalC1.g}, ${finalC1.b}, 0.12)`,
    color2: `rgba(${finalC2.r}, ${finalC2.g}, ${finalC2.b}, 0.04)`,
  };
};
  ...
  useEffect(() => {
    ...
    const renderLoop = () => {
      ...
      if (currentHour !== undefined) {
        const { color1, color2 } = getDayNightGradients(currentHour);
        dynamicGlow.addColorStop(0, color1);
        dynamicGlow.addColorStop(0.5, color2);
        dynamicGlow.addColorStop(1, 'rgba(0, 0, 0, 0)');
      } else if (isLightMode) {
        dynamicGlow.addColorStop(0, `rgba(${r}, ${g}, ${b}, 0.15)`);
        dynamicGlow.addColorStop(0.5, `rgba(${r}, ${g}, ${b}, 0.04)`);
        dynamicGlow.addColorStop(1, '#ffffff'); // Matches light background white exactly
      } else {
      ...
      // --- Draw Realistic Shooting Stars ---
      if (Math.random() < 0.003 * config.dilationFactor) {
        // Spawn from top mostly, falling diagonally
        const sx = Math.random() * width * 1.5 - width * 0.25;
        const sy = -50;
        const angle = Math.PI * 0.2 + (Math.random() * 0.15 - 0.075); // ~36 deg down right
        const speed = 2 + Math.random() * 3; // slower realistic speed
      ...
      shootingStars = shootingStars.filter(s => {
        s.life++;
        s.x += s.vx * config.dilationFactor;
        s.y += s.vy * config.dilationFactor;
        
        const progress = s.life / s.maxLife;
        const alpha = Math.sin(progress * Math.PI) * 0.8;
        if (alpha <= 0 && s.life > 10) return false;

        const totalSpeed = Math.sqrt(s.vx * s.vx + s.vy * s.vy);
        const trailLen = 40 + totalSpeed * 15;
        const trailX = s.x - (s.vx / totalSpeed) * trailLen;
        const trailY = s.y - (s.vy / totalSpeed) * trailLen;

        // Long gradient trail
        const grad = ctx.createLinearGradient(s.x, s.y, trailX, trailY);
        grad.addColorStop(0, isLightMode ? `rgba(${r}, ${g}, ${b}, ${alpha})` : `rgba(255, 255, 255, ${alpha})`);
        grad.addColorStop(0.1, isLightMode ? `rgba(255, 210, 140, ${alpha * 0.7})` : `rgba(180, 220, 255, ${alpha * 0.7})`);
        grad.addColorStop(1, isLightMode ? 'rgba(255, 255, 255, 0)' : 'rgba(0, 0, 0, 0)');
        ...
        glowGrad.addColorStop(0, isLightMode ? `rgba(${r}, ${g}, ${b}, ${alpha})` : `rgba(255, 255, 255, ${alpha})`);
        glowGrad.addColorStop(0.2, isLightMode ? `rgba(255, 230, 180, ${alpha * 0.6})` : `rgba(150, 200, 255, ${alpha * 0.5})`);
        glowGrad.addColorStop(1, isLightMode ? 'rgba(255, 255, 255, 0)' : 'rgba(0, 0, 0, 0)');
      ...
      const ringColors = {
        titanium: isLightMode ? 'rgba(30, 40, 60, 0.15)' : 'rgba(150, 150, 150, 0.12)',
        obsidian: isLightMode ? 'rgba(30, 40, 60, 0.25)' : 'rgba(80, 80, 80, 0.25)',
        stellar: isLightMode ? 'rgba(255, 62, 0, 0.3)' : 'rgba(255, 62, 0, 0.25)',
        polymer: isLightMode ? 'rgba(0, 180, 220, 0.3)' : 'rgba(0, 221, 255, 0.25)',
      };
      
      const ringAccentColors = {
        titanium: isLightMode ? 'rgba(30, 40, 60, 0.65)' : 'rgba(255, 255, 255, 0.5)',
        obsidian: isLightMode ? 'rgba(30, 40, 60, 0.8)' : 'rgba(140, 140, 150, 0.7)',
        stellar: 'rgba(255, 62, 0, 0.8)',
        polymer: isLightMode ? 'rgba(0, 140, 200, 0.8)' : 'rgba(0, 221, 255, 0.8)',
      };
        ...
        if (idx === 2) {
          ...
          for (let m = 0; m < indexCount; m++) {
            ...
            if (mode === 'TEMPORAL' && m % 6 === 0) {
              ...
              ctx.fillStyle = isLightMode ? 'rgba(30, 40, 60, 0.65)' : 'rgba(255, 255, 255, 0.4)';
      ...
      if (mode === 'AESTHETIC') {
        ...
        for (let i = 0; i < nodes.length; i++) {
          ...
          gradient.addColorStop(0, isLightMode ? 'rgba(30, 40, 60, 0.05)' : 'rgba(255, 255, 255, 0.05)');
          gradient.addColorStop(0.5, firingIntensity > 0.6 ? config.laserColor : (isLightMode ? 'rgba(30, 40, 60, 0.05)' : 'rgba(255, 255, 255, 0.05)'));
          gradient.addColorStop(1, isLightMode ? 'rgba(30, 40, 60, 0.05)' : 'rgba(255, 255, 255, 0.05)');
        ...
        nodes.forEach((node) => {
          ...
          ctx.fillStyle = node.firing > 0.75 ? (isLightMode ? '#1e293b' : '#ffffff') : config.laserColor;
          ctx.fill();
        ...
        for (let s = 0; s < constellationCount; s++) {
          ...
          if (pt.visible) {
            ...
            ctx.beginPath();
            ctx.strokeStyle = isLightMode ? 'rgba(30, 40, 60, 0.15)' : 'rgba(255,255,255,0.15)';
            ctx.lineWidth = 0.5;
            ...
            ctx.fillStyle = isLightMode ? 'rgba(30, 40, 60, 0.65)' : 'rgba(255,255,255,0.45)';
        ...
        ctx.beginPath();
        ctx.strokeStyle = isLightMode ? 'rgba(30, 40, 60, 0.05)' : 'rgba(255, 255, 255, 0.04)';
        ctx.lineWidth = 0.8;
      ...
      particles.forEach((p) => {
        ...
        if (mode === 'NEURAL') {
          ctx.fillStyle = isLightMode ? `rgba(30, 40, 60, ${baseAlpha * 0.85})` : `rgba(255, 255, 255, ${baseAlpha * 0.95})`;
        } else {
          ctx.fillStyle = `rgba(${r}, ${g}, ${b}, ${baseAlpha * (isLightMode ? 0.75 : 0.85)})`;
        }
        ctx.fill();
      ...
      ctx.strokeStyle = isLightMode ? 'rgba(30, 40, 60, 0.05)' : 'rgba(255, 255, 255, 0.02)';
      ctx.lineWidth = 1;
      ...
      ctx.fillStyle = isLightMode ? 'rgba(30, 40, 60, 0.5)' : 'rgba(255, 255, 255, 0.25)';
      ctx.textAlign = 'left';
